...
void SnesSystem::ExecuteCPU(Int32 nCycles)
{
    ...
    while (m_Cpu.Cycles > 0)
    {
        ...
        if (m_SA1.IsActive())
        {
            ...
            {
                Int32 nForce = before;
                if (nForce > SNCPU_CYCLE_FAST)
                    nForce = SNCPU_CYCLE_FAST;
                if (nForce > 0)
                    SNCPUConsumeCycles(&m_Cpu, nForce);
            }

            nSpent = before - m_Cpu.Cycles;
            if (nSpent < 0) nSpent = 0;
            if (nSpent > before) nSpent = before;
