...
print("====================================================")
print(" SNESticle Aurora - Top Gear / QuickNES / soft reset")
print("====================================================\n")
...
if flag not in s:
    anchor = "\t-DSNPPU_BG_CACHE=$(SNES_BG_CACHE) \\\n"
    count = s.count(anchor)
    if count != 2:
        die(
            "Makefile: esperava 2 ocorrências de SNPPU_BG_CACHE em "
            f"CFLAGS/CXXFLAGS, encontrei {count}"
        )
    s = s.replace(
        anchor,
        anchor + "\t-DSNPPU_CHR_CACHE_HFLIP=$(SNES_CHR_HFLIP_CACHE) \\\n",
    )
