...
/*
 * Pointer-free snapshot added by SNESticleRevive.
 *
 * Nes_Snd_Emu has public save_state()/load_state() declarations, but the
 * upstream library never shipped the matching apu_state_t definition or
 * implementation.  The PS2 frontend needs a stable state that does not copy
 * Blip_Buffer pointers, synth references, callbacks, or allocation metadata.
 * Audio already queued in Blip_Buffer is intentionally discarded on load;
 * oscillator phase, envelopes, length counters and DMC playback state are
 * retained and resume from a fresh output block.
 */
#define NES_APU_STATE_MAGIC   0x42415055u /* "BAPU" */
#define NES_APU_STATE_VERSION 1u

struct apu_osc_state_t {
	uint8_t regs[4];
	uint8_t reg_written[4];
	int32_t length_counter;
	int32_t delay;
	int32_t last_amp;
};

struct apu_envelope_state_t {
	apu_osc_state_t osc;
	int32_t envelope;
	int32_t env_delay;
};

struct apu_square_state_t {
	apu_envelope_state_t envelope;
	int32_t phase;
	int32_t sweep_delay;
};

struct apu_triangle_state_t {
	apu_osc_state_t osc;
	int32_t phase;
	int32_t linear_counter;
};

struct apu_noise_state_t {
	apu_envelope_state_t envelope;
	int32_t noise;
};

struct apu_dmc_state_t {
	apu_osc_state_t osc;
	int32_t address;
	int32_t period;
	int32_t buf;
...
struct apu_state_t {
	...
	uint32_t version;
	double tempo;
	apu_square_state_t square1;
	apu_square_state_t square2;
	apu_triangle_state_t triangle;
	apu_noise_state_t noise;
	apu_dmc_state_t dmc;
	int32_t next_irq_delay;
...
// Additional optional features (can be ignored without any problem)
	...
	// Run the complete APU (all five oscillators and the frame sequencer)
	// to the specified time without ending the current host frame. This
	// integration hook lets the save-state layer snapshot between writes
	// without clearing the frame IRQ flag.
	void synchronize( nes_time_t time ) { run_until_( time ); }
