...
NES_EMU_CPP = EMU / "Nes_Emu.cpp"
WRAPPER_NESROM = ROOT / "src/nes/system/nesrom.cpp"
QN_BRIDGE_CPP = ROOT / "src/nes/quicknes/quicknes_bridge.cpp"
QN_BRIDGE_H = ROOT / "src/nes/quicknes/quicknes_bridge.h"
QN_SYSTEM_CPP = ROOT / "src/nes/quicknes/nessystem_quicknes.cpp"
MAINLOOP_STATE_CPP = ROOT / "src/platform/ps2/system/mainloop_state.cpp"
...
MARK_WRAPPER = "AURORA_NESROM_NES2_GATE_V1"
MARK_PS2_STATE = "AURORA_PS2_DYNAMIC_NES_STATE_V1"
...
for p in [CART_H, CART_CPP, MAPPER_CPP, PPU_IMPL_CPP, NES_EMU_CPP,
          WRAPPER_NESROM, QN_BRIDGE_CPP, QN_BRIDGE_H, QN_SYSTEM_CPP,
          MAINLOOP_STATE_CPP, M088, M154, M206]:
    if not p.exists():
...
original = {
    ...
    NES_EMU_CPP: read(NES_EMU_CPP),
    WRAPPER_NESROM: read(WRAPPER_NESROM),
    QN_BRIDGE_CPP: read(QN_BRIDGE_CPP),
    QN_BRIDGE_H: read(QN_BRIDGE_H),
    QN_SYSTEM_CPP: read(QN_SYSTEM_CPP),
    MAINLOOP_STATE_CPP: read(MAINLOOP_STATE_CPP),
    M088: read(M088),
    M154: read(M154),
    M206: read(M206),
}
...
# ---------------------------------------------------------------------------
# 0A) SNESticle outer .nes wrapper: NES 2.0 must pass before QuickNES.
# ---------------------------------------------------------------------------

s = modified[WRAPPER_NESROM]

wrapper_helper = r"""/* AURORA_NESROM_NES2_GATE_V1
 *
...
wrapper_fn = r"""Emu::Rom::LoadErrorE NesRom::LoadRom(
...
if MARK_WRAPPER not in s:
    a, b = function_span(s, "Emu::Rom::LoadErrorE NesRom::LoadRom(")
    s = s[:a] + wrapper_helper + wrapper_fn + s[b:]
else:
    s = replace_function(
        s,
...
modified[WRAPPER_NESROM] = s
print("[ OK ] SNESticle .nes gate: NES 2.0 / ROMs pequenas aceitas")


# ---------------------------------------------------------------------------
# 0B) Restore the PS2 QuickNES save-state fix that previously worked.
# ---------------------------------------------------------------------------

s = modified[NES_EMU_CPP]

# Upstream optimization uses file-scope static Nes_State scratch objects.
# On PS2, keep construction explicit at save/load time instead. Nes_State's
# constructor wires internal pointers to its own embedded buffers.
if MARK_PS2_STATE not in s:
    if ("#ifdef PS2" in s and
        s.count("new Nes_State") >= 2 and
        "s_save_state_scratch" in s and
        "s_load_state_scratch" in s):
        print("[PASS] QuickNES save/load: PS2 já usa Nes_State dinâmico")
    else:
        static_block = """static Nes_State s_save_state_scratch;
static Nes_State s_load_state_scratch;
"""
        guarded_block = """/* AURORA_PS2_DYNAMIC_NES_STATE_V1
 * The PS2 path constructs Nes_State explicitly for every file snapshot.
 * Non-PS2 keeps QuickNES's static scratch optimization.
 */
#ifndef PS2
...
"""
        if static_block not in s:
            die("Nes_Emu.cpp: static savestate scratch esperado não encontrado")
        s = s.replace(static_block, guarded_block, 1)

        load_old = """const char * Nes_Emu::load_state( Auto_File_Reader in )
{
...
"""
        load_new = """const char * Nes_Emu::load_state( Auto_File_Reader in )
{
#ifdef PS2
...
"""
        if load_old not in s:
            die("Nes_Emu.cpp: load_state scratch antigo não encontrado")
        s = s.replace(load_old, load_new, 1)

        save_old = """const char * Nes_Emu::save_state( Auto_File_Writer out ) const
{
...
"""
        save_new = """const char * Nes_Emu::save_state( Auto_File_Writer out ) const
{
#ifdef PS2
	Nes_State* state = new Nes_State;
	CHECK_ALLOC( state );
...
"""
        if save_old not in s:
            die("Nes_Emu.cpp: save_state scratch antigo não encontrado")
        s = s.replace(save_old, save_new, 1)


modified[NES_EMU_CPP] = s
print("[ OK ] QuickNES save/load: Nes_State dinâmico restaurado no PS2")

# Verify the already-known frontend half of commit b89bba1. If a local merge
# removed it, abort before writing rather than silently reintroducing the old
# double-serialization path.
bridge_h = modified[QN_BRIDGE_H]
bridge_cpp = modified[QN_BRIDGE_CPP]
system_cpp = modified[QN_SYSTEM_CPP]
mainloop_state = modified[MAINLOOP_STATE_CPP]

frontend_checks = [
    (bridge_h, "QUICKNES_STATE_CAPACITY = 32 * 1024", "bridge capacity"),
    (bridge_h, "int QuicknesBridge_SaveState", "bridge SaveState return type"),
    (bridge_cpp, "return s_GameLoaded ? QUICKNES_STATE_CAPACITY : 0;", "no state-size probe"),
    (bridge_cpp, "long written = writer.size();", "single serialization result"),
    (system_cpp, "sizeof(QuicknesStateHeaderT) + QUICKNES_STATE_CAPACITY", "compact envelope"),
    (system_cpp, "snapshot committed", "header-last save"),
    (mainloop_state, "_pNes->GetStateSize()", "active NES payload size"),
]
for text, needle, label in frontend_checks:
    if needle not in text:
        die("save-state frontend regrediu/faltando: " + label)

print("[PASS] save-state frontend anterior (b89bba1) continua presente")
...
checks = [
    (modified[WRAPPER_NESROM], MARK_WRAPPER, "SNESticle NES2 outer gate"),
    (modified[NES_EMU_CPP], MARK_PS2_STATE, "PS2 dynamic Nes_State"),
...
try:
    ...
    main_paths = [
        ...
        for p in targets
        if QN not in p.parents
    ]
    if main_paths:
        r = run(["git", "diff", "--check", "--"] + main_paths, cwd=ROOT)
        if r is None or r.returncode != 0:
            raise RuntimeError(
                "git diff --check falhou no Aurora:\n" +
                (r.stdout if r else "timeout")
            )
...
print("  - .nes outer loader: NES 2.0 aceito antes do QuickNES")
print("  - QuickNES save/load PS2: Nes_State dinâmico restaurado")
...
print("  0. Donkey Kong (mapper 0)              -> sanity de .nes/NROM")
