...
void _MainLoopInputProcess(Uint32 buttons)
{
    ...
    static Bool bFrontCopierCloseHotkeyHeld = FALSE; /* AURORA_FRONT_COPIER_L2_R3_CLOSE_V1_20260903 */
    static Bool bFrontCopierPowerHotkeyHeld = FALSE;

    Uint32 trigger;
    /* AURORA_INPUT_SUPPRESS_ALL_GAMEPLAY_V1_4_3
     * Host inputs consumed by menu/copier transitions stay suppressed until
     * release; this is independent of emulated controller state. */
    if (_MainLoop_bSuppressGameInputUntilRelease &&
    ...
    if (!(buttons & PAD_L2) || !(buttons & PAD_R3))
        bFrontCopierCloseHotkeyHeld = FALSE;

    /* AURORA_FRONT_COPIER_L2_R3_CLOSE_V1_20260903
     * AURORA_SMD_L2_R3_CLOSE_V1_20260903 */
    if (!_bMenu &&
        (((_pSystem == _pSnes) && _pSnes && _pSnes->IsSuperWildCard()) ||
         ((_pSystem == _pSega) && _pSega && _pSega->IsSuperMagicDrive())) &&
        !bFrontCopierCloseHotkeyHeld &&
        (buttons & (PAD_L2 | PAD_R3)) == (PAD_L2 | PAD_R3) &&
        !(buttons & PAD_R2) && (trigger & (PAD_L2 | PAD_R3)))
    {
        bFrontCopierCloseHotkeyHeld = TRUE;
    ...
    if (!_bMenu && _pSystem == _pSega && _pSega->IsSuperMagicDrive() && !bFrontCopierPowerHotkeyHeld &&
        (buttons & (PAD_L2|PAD_L3)) == (PAD_L2|PAD_L3) && !(buttons & PAD_R2) && (trigger & (PAD_L2|PAD_L3)))
    {
        bFrontCopierPowerHotkeyHeld=TRUE; _MenuTriggerTimeout[0]=_MenuTriggerTimeout[1]=0;
        _MainLoopInputSuppressUntilRelease(); _pSega->PowerCycleSuperMagicDrive(); return;
    }
    ...
    /* AURORA_SUPER_MAGIC_DRIVE_V1_20260902 */
    if (!_bMenu && _pSystem == _pSega && _pSega->IsSuperMagicDrive() && !bSwcCreateHotkeyHeld &&
        (buttons & PAD_L2) && (buttons & PAD_SQUARE) && !(buttons & PAD_R2) && (trigger & (PAD_L2|PAD_SQUARE)))
    { bSwcCreateHotkeyHeld=TRUE; _MenuTriggerTimeout[0]=_MenuTriggerTimeout[1]=0; MainLoopSmdCreateNextDisk(); return; }
    ...
    /* AURORA_SUPER_MAGIC_DRIVE_V1_20260902 */
    if (!_bMenu && _pSystem == _pSega && _pSega->IsSuperMagicDrive() && !bSwcSwapHotkeyHeld &&
        (buttons & PAD_L2) && (buttons & PAD_TRIANGLE) && !(buttons & PAD_R2) && (trigger & (PAD_L2|PAD_TRIANGLE)))
    { bSwcSwapHotkeyHeld=TRUE; _MenuTriggerTimeout[0]=_MenuTriggerTimeout[1]=0; MainLoopSmdSwapNextDisk(); return; }
