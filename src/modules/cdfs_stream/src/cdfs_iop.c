...
int cdfs_dirStreamOpen(const char *pathname, struct CdfsDirStream *stream) {
    u32 cached_sectors;
    ...
    /* Match cdfs_getDir's two-step path resolution. The first call locates
       the requested path; the second pins the cache to that directory. */
    if (!cdfs_cacheDir((char *)pathname, CACHE_START))
        return -1;
    if (!cdfs_cacheDir(cacheInfoDir.pathname, CACHE_START))
        return -1;

    stream->sector_start = cacheInfoDir.sector_start;
    stream->sector_count = cacheInfoDir.sector_num;
    stream->byte_offset = 0;
    stream->cache_sector = cacheInfoDir.cache_offset;
    stream->skip_root_parent = (cacheInfoDir.path_depth == 0);

    cached_sectors = cacheInfoDir.cache_size;
    if (cached_sectors > CDFS_DIR_STREAM_CACHE_SECTORS)
        cached_sectors = CDFS_DIR_STREAM_CACHE_SECTORS;
    stream->cache_count = cached_sectors;
    if (cached_sectors > 0)
        memcpy(stream->cache, cacheInfoDir.cache, cached_sectors * 2048);

    return 0;
}

int cdfs_dirStreamRead(struct CdfsDirStream *stream, struct TocEntry *tocEntry) {
    u32 total_bytes;

    if (stream == NULL || tocEntry == NULL)
        return -1;

    total_bytes = stream->sector_count * 2048;
    while (stream->byte_offset < total_bytes) {
        u32 sector = stream->byte_offset >> 11;
        u32 in_sector = stream->byte_offset & 0x7ff;
        struct DirTocEntry *internal;
        u32 remaining;
        u32 to_read;

        if (stream->cache_count == 0 ||
            sector < stream->cache_sector ||
            sector >= stream->cache_sector + stream->cache_count) {
            remaining = stream->sector_count - sector;
            to_read = remaining;
            if (to_read > CDFS_DIR_STREAM_CACHE_SECTORS)
                to_read = CDFS_DIR_STREAM_CACHE_SECTORS;

            if (to_read == 0 ||
                !cdfs_readSect(stream->sector_start + sector,
                               to_read, stream->cache)) {
                stream->cache_count = 0;
                return -1;
            }
            stream->cache_sector = sector;
            stream->cache_count = to_read;
        }

        internal = (struct DirTocEntry *)(stream->cache +
                   ((sector - stream->cache_sector) * 2048) + in_sector);

        /* ISO9660 records never cross a sector. A zero length pads the
           remainder of the current sector. */
        if (internal->length == 0) {
            stream->byte_offset = (sector + 1) * 2048;
            continue;
        }
        if (internal->length < 34 ||
            in_sector + (u32)internal->length > 2048) {
            stream->byte_offset = (sector + 1) * 2048;
            continue;
        }

        stream->byte_offset += (u32)internal->length;

        /* Skip the ISO self entry. Preserve the parent record as "..";
           the EE browser already filters it, and other clients expect it. */
        if (internal->filenameLength == 1 && internal->filename[0] == 0)
            continue;

        if (internal->filenameLength == 1 && internal->filename[0] == 1 &&
            stream->skip_root_parent)
            continue;

        copyToTocEntry(tocEntry, internal);
        if (internal->filenameLength == 1 && internal->filename[0] == 1)
            strcpy(tocEntry->filename, "..");
        return 1;
    }
